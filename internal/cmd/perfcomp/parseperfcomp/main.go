...
func main() {
	...
	for scanner.Scan() {
		...
		// parse commit SHA
		if strings.Contains(line, "Commit SHA:") {
			fmt.Fprintf(fWrite, "\n<details>\n<summary>%s</summary>\n\t<br>\n\n", line)
			continue
		}
		...
		// last line
		if strings.Contains(line, "For a comprehensive view of all microbenchmark results for this PR's commit, please check out the Evergreen perf task for this patch.") {
